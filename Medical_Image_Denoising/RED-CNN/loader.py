...
class ct_dataset(Dataset):
    def __init__(self, mode, load_mode, saved_path, test_patient, patch_n=None, patch_size=None, transform=None):
        ...
        load_mode：加载模式，可以是 0 或 1，决定了数据是按批次加载还是一次性加载到内存。
        saved_path：保存Numpy文件的路径。
        test_patient：测试患者的标识，用于区分训练和测试数据。
        patch_n 和 patch_size：用于数据增强的参数，分别表示补丁数量和补丁大小。
        transform：可选的变换函数，用于对图像进行预处理。
        '''
